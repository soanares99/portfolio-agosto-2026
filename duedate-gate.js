const COOKIE_NAME = "case_duedate_auth";
const ENV_VAR = "PROTECTED_PAGE_PASSWORD_DUEDATE";
...
<title>Change Payment Due Date — Natália Amaral</title>
  ...
  if (request.method === "POST") {
    ...
    if (submittedHash === expectedHash) {
      const response = new Response(null, {
        ...
        headers: {
          "Location": url.origin + url.pathname,
          "Set-Cookie": `${COOKIE_NAME}=${expectedHash}; Path=/; Max-Age=86400; HttpOnly; Secure; SameSite=Strict`,
        },
      });
      return response;
    }
...
export const config = { path: "/case-due-date.html" };
